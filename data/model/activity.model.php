...
class activityModel{
	 ...
	 * @return array 二维数组
	 */
	public function getListAndItemCount($condition,$page=''){
		$param	= array();
		$param['table']	= 'activity';
		$param['where']	= $this->getCondition($condition);
		$param['order']	= $condition['order'] ? $condition['order'] : 'activity_id';
...
//                exit();
                foreach ($list as $key => $value) {
                    $condition2['activity_id']=$value['activity_id'];
                    $condition2['activity_detail_state']=1;
                    $count = Db::getCount('activity_detail',$condition2);
                   // echo($count['count'].'fuck');
                    $list[$key]['detail_count']=$count;
                }
                return $list;
	}
	 ...
	 * @return bool
	 */
	public function updateBycondition($input,$condition){
                
                $condition=$this->getCondition($condition);
                
		return Db::update('activity',$input,$condition);
	...
	private function getCondition($condition){
		...
		if($condition['activity_id'] != ''){
			$conditionStr	.= " and activity.activity_id='{$condition['activity_id']}' ";
                ...
                if($condition['activity_funder'] != ''){
			$conditionStr	.= " and activity.activity_store_id > 0 ";
		}
		...
		if($condition['activity_state'] != ''){
			$conditionStr	.= " and activity.activity_state = '{$condition['activity_state']}' ";
                ...
                if($condition['store_id']!=''){
                    $conditionStr	.= " and activity.activity_store_id = '{$condition['store_id']}' ";
                }
		//活动删除in
     ...
     * @return boolean
     */
    public function editActivityVerifyFail($condition, $update1, $update2 = array()) {
        $condition=$this->getCondition($condition);
        
        $result = Db::update('activity',$update1,$condition);
        
      //  $result = $this->editProduces($condition, $update1, $update2);
        if ($result) {
            
            $commonlist = $this->getList($condition);
