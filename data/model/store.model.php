...
class storeModel extends Model {
    ...
    protected $services=array('store_zhping'=>'正品保障','store_qtian'=>"7天退货",'store_erxiaoshi'=>"两小时发货",'store_tuihuo'=>"退货承诺",'store_shiti'=>"实体验证",'store_xiaoxie'=>"消协保障",'store_huodaofk'=>"货到付款",'store_baozhrmb'=>"诚信金保证");
     ...
     * @return array
	 */
    public function getDcStoreOnlineIdArray($condition, $page = null, $order = '', $field = '*') {
        $condition=$this->_dealCondition($condition);
        $order=$this->_dealDcOrder($order);
        $condition['store_state'] = 1;
        $condition['store_apply_type'] = 2;
       
        return $this->getStoreList($condition, $page, $order, $field);
    }
    
    public function _dealDcOrder($param) {
        if($param=='1'){
            return 'rand()';
        }else if($param==2){
     ...
     * @return int
     */
    public function getStoreCount2($condition) {
        $condition=$this->_dealCondition($condition);
        return $this->where($condition)->count();
    }
     ...
     * @return int
     */
    public function getDcStoreCount($condition) {
        $condition=$this->_dealCondition($condition);
        $condition['store_apply_type']=2;
        return $this->where($condition)->count();
    }
    ...
    public function getStoreInfo($condition) {
        ...
        if(!empty($store_info)) {
            ...
            $store_info['store_logo'] = getStoreLogo($store_info['store_avatar']);
            //店铺评价
            ...
            $store_info['store_services']=$servicesArray;
    	...
    	if (!empty($list) && is_array($list)){
    		...
    		if($day > 0) {
    			...
    			$list_new = $this->getGoodsCountByStoreArray($list_new);
                 ...
                 $list_new = $this->getGoodsListBySales($list_new);
                  $list_new = $this->getGoodsCountJq($list_new);
    	}
    	return $list_new;
    }
    
    /**
     * 获得装修公司标志、案例数量、收藏，设计师数量，评分等信息
     *
     * @param	array $param 店铺数组
     * @return	array 数组格式的返回结果
     */
    public function getDcInfoBasic($list,$day = 0){
    	$list_new = array();
    	if (!empty($list) && is_array($list)){
    		foreach ($list as $key=>$value) {
    			if(!empty($value)) {
...
//    		}
                 ...
                 $list_new = $this->getWorkBy($list_new);
                 $list_new = $this->getDesignerBy($list_new);
                //  $list_new = $this->getDcDealCount($list_new);
    	}
    	return $list_new;
    }
    
      //获取近期成交量
    private function getDcDealCount($store_array) {
    	$model = Model();
    	$order_count_array = $model->table('bid')->field('bid_store_id,count(*) as deal_count')->where(array('bid_state'=>10,'bid_store_id'=>array('in',implode(',',array_keys($store_array))),'bid_addtime'=>array('gt',TIMESTAMP-3600*24*90)))->group('bid_store_id')->select();
    	
        foreach ((array)$order_count_array as $value) {
    		$store_array[$value['store_id']]['num_deals_jq'] = $value['deal_count'];
    	}
    ...
    //获取  作品
    private function getWorkBy($store_array) {
    	$model = Model();
    	$field = '*';
    	foreach ($store_array as $value) {
                $condition['designer_work.sn_store_id']=intval($value['store_id']);
                $condition['designer_work.sn_if_show']=1;
    		$store_array[$value['store_id']]['search_list_works'] = Model('designer_work')->getDesignerWorkListwebNoPage($condition);
                    
                $condition1['sn_store_id']=intval($value['store_id']);
                $count = $model->table('designer_work')->where($condition1)->count();
                $store_array[$value['store_id']]['work_count']=$count;
       ...
       //获取 最高设计师
    private function getDesignerBy($store_array) {
    	$model = Model();
    	$field = '*';
    	foreach ($store_array as $value) {
                
    		$store_array[$value['store_id']]['search_list_designers'] = $model->table('designer')->field($field)->where(array('sn_store_id'=>intval($value['store_id']),'sn_if_show'=>1))->order('sn_designer_collect desc')->limit(8)->select();
                //$store_array[$value['store_id']]['search_list_designers']['work_count'] = $model->table('designer_work')->field($field)->where(array('sn_store_id'=>intval($value['store_id']),'sn_if_show'=>1,'sn_designer_id'=>))->order('sn_designer_collect desc')->limit(8)->select();
                $condition['sn_store_id']=$value['store_id'];
                $count = $model->table('designer')->where($condition)->count();
                $store_array[$value['store_id']]['designer_count']=$count;
        }
