    ...
    public function getCache() {
       ...
       // $data = rkcache('gc_class');
        ...
        if (!$data) {
            ...
            $data = array();
            foreach ((array) $this->getGoodsClassList2(array()) as $v) {
                $id = $v['gc_id'];
                $pid = $v['gc_parent_id'];
                $data['data'][$id] = $v;
                $data['parent'][$id] = $pid;
...
//        exit();
     ...
     * @return array   返回二位数组
     */
    public function getGoodsClassList2($condition, $field = 'gc_id,gc_name,gc_parent_id') {
        $result = $this->table('goods_class')->field($field)->where($condition)->order('gc_parent_id asc,gc_sort asc,gc_id asc')->limit(false)->select();
        return $result;
    }
     ...
     * 从缓存获取全部分类
     */
    public function getGoodsClassListAll2() {
        $data = $this->getCache2();
        return array_values((array) $data['data']);
    }
     ...
     * @return  array   数组
     */
    public function get_all_category2($update_all = 0) {
            ...
            if (is_array($class_list) && !empty($class_list)) {
                foreach ($class_list as $key => $value) {
                    ...
                    if ($p_id == 0) {//第1级分类
                        ...
                        $class2_ids[$gc_id] = $p_id;
                        $type_ids[] = $value['type_id'];
                        $gc_list[$p_id]['children'][$gc_id] = $value;
                    } elseif (array_key_exists($p_id,$class2_ids)) {//第3级
                        $parent_id = $class2_ids[$p_id];//取第1级ID
                        $gc_list[$parent_id]['children'][$p_id]['children'][$gc_id] = $value;
                        $class1_deep[$parent_id][$sort][] = $value;
                    }
                }
//                var_dump($gc_list);
...
//                }
            ...
            foreach ($gc_list as $key=>$val) {
                foreach ($val['children'] as $key2 => $value2) {
                     ...
                     $gc_list[$key]['children'][$key2]['children']= array_values($value2['children']);
                     
                }
            }
            foreach ($gc_list as $key=>$val) {
                $gc_list[$key]['children']= array_values($val['children']);
            }
