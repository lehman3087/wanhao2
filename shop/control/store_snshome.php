...
class store_snshomeControl extends BaseStoreSnsControl{
	 ...
	 * 评论前10条记录
	 */
	private function mobilecommenttop(){
...
// 		//查询会员信息
	...
	public function straceinfoOp(){
        ...
        public function mobileStraceinfoOp(){
		$stid = intval($_GET['st_id']);
		...
		$strace_info = $model_stracelog->getStoreSnsTracelogInfo(array('strace_id' => $stid));
                ...
                $update['strace_click']=array('exp','1strace_click+1');
                $conditionclick['strace_id']=$stid;
                $model_stracelog->editStoreSnsTracelog($update,$conditionclick);
		...
		Tpl::output('nchash',substr(md5(SHOP_SITE_URL.$_GET['act'].$_GET['op']),0,8));
                ...
                $this->mobilecommenttop();
                
		Tpl::showpage('store_mobilesnstraceinfo','mobile_layout');
	}
